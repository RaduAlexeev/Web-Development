...
// Determine if the poker hand is flush, meaning if the five cards are of 
// the same suit.
...
// card followed by the initial of its suit (Hearts, Spades, Diamonds or 
//     Clubs). No jokers included.
...
function isFlush(cards) {
    ...
    for (let suit of suits) {
        if (cards.every((card) => card.includes(suit))) {
            return true
        }
    }
    return false
